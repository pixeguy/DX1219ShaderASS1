Shader "Custom/DeferredLighting"
{
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 _DLightPositions[MAX_LIGHTS];  
            float4 _DLightDirections[MAX_LIGHTS];   
            float4 _DLightColors[MAX_LIGHTS];     
            float4 _DLightIntensities[MAX_LIGHTS];  
            float4 _DLightTypes[MAX_LIGHTS];        
            float4 _DLightAttenuations[MAX_LIGHTS]; 
            float4 _DSpotAngles[MAX_LIGHTS];        // x = inner angle, y = outer angle
            float4 _DLightSpecularStrength[MAX_LIGHTS]; 
            float4 _DLightSmoothness[MAX_LIGHTS];      
            float4 _DLightRange[MAX_LIGHTS];       
            ...
            float4 Frag(Varyings i) : SV_Target
            {
                ...
                for (int idx = 0; idx < MAX_LIGHTS; idx++)
                {
                    ...
                    int type = (int)_DLightTypes[idx].x;

                    float3 Lpos    = _DLightPositions[idx].xyz;
                    float3 Ldir    = normalize(_DLightDirections[idx].xyz);
                    float3 Lcolor  = _DLightColors[idx].rgb;
                    float  Lintens = _DLightIntensities[idx].x;

                    float3 atten   = _DLightAttenuations[idx].xyz;
                    float  inner   = radians(_DSpotAngles[idx].x);
                    float  outer   = radians(_DSpotAngles[idx].y);
                    float range = _DLightRange[idx].x;
                    ...
                    float Lsmooth = _DLightSmoothness[idx].x;
                    float LspecStrength = _DLightSpecularStrength[idx].x;
