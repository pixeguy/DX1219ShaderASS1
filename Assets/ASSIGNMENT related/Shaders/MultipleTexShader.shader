Shader "Custom/MultipleTexShader"
{
    Properties
    {       
        ...
        _fourthTexture ("Texture",2D) = "white" {}
    ...
    { 
        ...
        Pass
        {
            ...
            float4 MyFragmentShader(vertex2Fragment v2f) : SV_TARGET
            {
                ...
                float4 mask = tex2D(_fourthTexture, v2f.uv);
                float4 tex1 = tex2D(_mainTexture, v2f.uv);
                float4 tex2 = tex2D(_secTexture, v2f.uv);
                float4 tex3 = tex2D(_thirdTexture, v2f.uv);

                float4 albedo = _tint * (tex1 * mask.x + tex2 * mask.y + tex3 * mask.z);
