Shader "Custom/WaterShader2"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            // ======================================================
            // Water RTs / Gradients / Refraction / Foam
            // ======================================================
            ...
            // Reflection (declared but not used in your current fragment)
            ...
            float CalculateShadow(int lightIndex, float4 fragPosLightSpace)
            {
                ...
                for (int x = -2; x <= 2; x++)
                {
                    ...
                    for (int y = -2; y <= 2; y++)
                    {
                        float2 uv = atlasUV + float2(x, y) * texel;
            ...
            float4 MyFragmentShader(vertex2Fragment v2f) : SV_TARGET
            {
                ...
                float4 interfoamFinalCol = float4(interfoamCol.rgb, foamAlpha);
